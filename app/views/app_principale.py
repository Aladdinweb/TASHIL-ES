...
S'affiche dans la fenêtre racine AppRoot
après l'activation.
"""
...
from app.utils.database import (
    faire_backup, get_config)
from app.utils.version import get_version


class AppPrincipale(ctk.CTkFrame):
    """
    Frame qui contient sidebar + zone de contenu.
    Injecté dans AppRoot après activation.
    """
    ...
    def _construire_sidebar(self):
        ...
        f_txt = ctk.CTkFrame(
            frame_logo, fg_color="transparent")
    ...
    def _creer_vue(self, cle: str):
        ...
        from app.views.vue_bordereaux import (
            VueBordereaux)
                ...
                if version_plus_recente(tag):
                    self.after(0, lambda:
                        messagebox.showinfo(
                            ...
                            "Rendez-vous dans Releases."))
                else:
                    self.after(0, lambda:
                        messagebox.showinfo(
                            "Mise à jour",
                            f"✅ Version à jour : {tag}"))
