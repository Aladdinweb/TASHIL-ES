...
class FenetrePrincipale(ctk.CTk):
    ...
    def _construire_sidebar(self):
        ...
        badge = ctk.CTkFrame(frame_logo, fg_color=COULEURS["accent_bleu"],
                             corner_radius=8, width=44, height=44)
        badge.pack(side="left")
        badge.pack_propagate(False)
        ctk.CTkLabel(badge, text="ES", font=("Segoe UI", 16, "bold"),
                     text_color="#FFFFFF").place(relx=0.5, rely=0.5,
                                                 anchor="center")
        f_txt = ctk.CTkFrame(frame_logo, fg_color="transparent")
        f_txt.pack(side="left", padx=(10, 0))
        ctk.CTkLabel(f_txt, text="EPSP",
                     font=POLICES["titre_app"],
                     text_color=COULEURS["texte_principal"]).pack(anchor="w")
        ctk.CTkLabel(f_txt, text="ES-SENIA",
                     font=POLICES["petit"],
        ...
        for cle, libelle, icone in self.ONGLETS:
            # Séparateur avant Administration
            if cle == "admin":
                ctk.CTkFrame(sidebar, height=1,
                             fg_color=COULEURS["bordure"]).pack(
    ...
    def _construire_zone_contenu(self):
        self.frame_contenu = ctk.CTkFrame(
            self, fg_color=COULEURS["bg_principal"], corner_radius=0)
    ...
    def _naviguer(self, cle: str):
        ...
        for k, btn in self._boutons_nav.items():
            if k == cle:
                btn.configure(fg_color=COULEURS["sidebar_active_bg"],
    ...
    def _verifier_rollover_auto(self):
        """Alerte discrète si le rollover est en attente."""
        from app.utils.deduction_engine import verifier_rollover_necessaire
        if verifier_rollover_necessaire():
            import datetime
            annee = datetime.date.today().year - 1
            self._boutons_nav["admin"].configure(
